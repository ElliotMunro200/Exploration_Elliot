...
class PPO():
    ...
    def update(self, rollouts):
        advantages = rollouts.returns[:-1] - rollouts.value_preds[:-1].gather(2, rollouts.option[:-1].unsqueeze(
            -1).long()).detach().squeeze(-1)
            ...
            for sample in data_generator:
                ...
                if self.use_clipped_value_loss:
                    value_pred_clipped = value_preds.gather(1, sample['option'].long().view(-1, 1)) + \
                                         (values.gather(1, sample['option'].long().view(-1, 1)) - value_preds.gather(1,
                                                                                                                     sample[
                                                                                                                         'option'].long().view(
                                                                                                                         -1,
                                                                                                                         1))).clamp(
        ...
        num_updates = self.ppo_epoch * self.num_mini_batch  # 4 * 2 (for "auto") = 8
