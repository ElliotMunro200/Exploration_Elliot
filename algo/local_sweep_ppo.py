...
class PPO():
    ...
    def __init__(
        ...
        self.actor_critic = actor_critic # the (A2C: Advantage used to update Actor) (PPO: Advantage used to update Actor, Advantage can come from anywhere). 
                                         #PPO can be used for policy optimization within the A2C framework.

        self.clip_param = clip_param
        self.ppo_epoch = ppo_epoch
        self.num_mini_batch = num_mini_batch #2 
    ...
    def update(self, rollouts):
        # this calculation here is key to understand!?
        advantages = rollouts.returns[:-1] - rollouts.value_preds[:-1,:,0]
        ...
        for e in range(self.ppo_epoch): #4
            ...
            if self.actor_critic.is_recurrent: # True
                data_generator = rollouts.recurrent_generator(
                        advantages, self.num_mini_batch) # yielding a generator containing 2 mini-batches of data. Each mini-batch has 2 processes worth of data (1 ep each).
            else:
                data_generator = rollouts.feed_forward_generator( 
                        advantages, self.num_mini_batch)

            for sample in data_generator: #iterating through the 2 samples/batches from the data_generator.
                ...
                values, action_log_probs, dist_entropy, _ = \
                        self.actor_critic.evaluate_actions(
                            sample['obs'], num_scenes, sample['rec_states'], sample['rgb'],
                ...
                if self.use_clipped_value_loss: #False
        ...
        num_updates = self.ppo_epoch * self.num_mini_batch # 4 * (num_processes//2 = 4/2 = 2) (for "auto") = 8
