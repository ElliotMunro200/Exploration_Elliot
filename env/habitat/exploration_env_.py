...
class Exploration_Env(habitat.RLEnv):
    ...
    def __init__(self, args, rank, config_env, config_baseline, dataset):
        ...
        if args.print_images or args.visualize:
            self.figure, self.ax = plt.subplots(1,4, figsize=(6*16/9, 6),
    ...
    def reset(self):
        ...
        fp_proj, self.map, fp_explored, self.explored_map, self.new_explored, self.frontier = \
    ...
    def step(self, action):
        ...
        self.info['fp_proj'] = self.map,
        self.info['fp_explored']= self.explored_map,
        ...
        if self.info['time'] >= args.max_episode_length:
            ...
            if self.args.eval == 1:
                savemat("exp_data/"+str(self.rank)+"-"+str(self.episode_no)+".mat", {"num_explored": self.num_explored,"num_forward": \
                        self.num_forward,"gt_area": self.gt_area})
    ...
    def get_short_term_goal(self, inputs):
        ...
        if inputs['active'] == False:
            output = np.zeros((args.goals_size + 2))
            return output
        ...
        goal = inputs['goal']
        goal_arbitrary = inputs['goal_arbitrary']
        
        self.change_goal_flag = inputs['change_goal']
        #goal[0] = 500
        #goal[1] = 20
        goal = pu.threshold_poses(goal, grid.shape)
        self.goal = goal
        self.goal_arbitrary = goal_arbitrary
        ...
        if self.args.eval == 1:
            ...
            gt_action, stg_x_gt, stg_y_gt, self.fmm_dist, goal_reached, new_goal = self._get_gt_action((1 - self.explorable_map), start,
                                            [int(goal[0]), int(goal[1])],
                                            planning_window, start_o_gt) 
    ...
    def update_visualize(self, option):
        ...
        if args.visualize or args.print_images:
            ...
            vis_grid = vu.get_colored_map(self.map,
                            ...
                            self.explored_map,
                            self.explorable_map,
                            self.frontier, self.change_goal_flag)
