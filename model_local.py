...
class Global_Policy(NNBase):
    ...
    def forward(self, inputs, rnn_hxs, visual_feature, masks, extras):
        ...
        # [enc(maps), emb(ori), enc(rgb)] = (512 + 8 + 512) = (1032)
        x = torch.cat((x, orientation_emb, visual_feature), 1)

        # (1032) --> (512)
        x = nn.ReLU()(self.linear1(x))
        # True by default. Defined by args.use_recurrent_global.
        # Executes nn.GRUCell (maintained hidden state, input->output) on x: torch.cat[enc(maps), emb(ori), enc(rgb)]
        # x = (512), rnn_hxs = (512)
